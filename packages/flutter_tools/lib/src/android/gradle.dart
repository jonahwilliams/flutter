...
class AndroidGradleBuilder implements AndroidBuilder {
  ...
  Future<void> buildGradleApp({
    ...
    if (localEngineInfo != null) {
      ...
      _logger
          .printTrace('Using local engine: ${localEngineInfo.engineOutPath}\n'
      ...
      command.add(
          '-Ptarget-platform=${_getTargetPlatformByLocalEnginePath(localEngineInfo.engineOutPath)}');
    } else if (androidBuildInfo.targetArchs.isNotEmpty) {
      final String targetPlatforms = androidBuildInfo.targetArchs
          .map(getPlatformNameForAndroidArch)
          .join(',');
  ...
  Future<void> _performCodeSizeAnalysis(
    ...
    final String archName =
        getNameForAndroidArch(androidBuildInfo.targetArchs.single);
    final BuildInfo buildInfo = androidBuildInfo.buildInfo;
