...
class DevicesCommand extends FlutterCommand {
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final List<Device> devices = await globals.deviceManager
            ?.refreshAllConnectedDevices(timeout: deviceDiscoveryTimeout) ??
        <Device>[];
  ...
  Future<void> _printDiagnostics() async {
    final List<String> diagnostics =
        await globals.deviceManager?.getDeviceDiagnostics() ?? <String>[];
  ...
  Future<void> printDevicesAsJson(List<Device> devices) async {
    globals.printStatus(const JsonEncoder.withIndent('  ')
