...
abstract class RunCommandBase extends FlutterCommand
  ...
  bool get dumpSkpOnShaderCompilation =>
      boolArgDeprecated('dump-skp-on-shader-compilation');
  bool get purgePersistentCache => boolArgDeprecated('purge-persistent-cache');
  bool get disableServiceAuthCodes =>
      boolArgDeprecated('disable-service-auth-codes');
  ...
  Future<DebuggingOptions> createDebuggingOptions(bool webMode) async {
    ...
    if (buildInfo.mode.isRelease) {
      return DebuggingOptions.disabled(
        ...
        hostname: featureFlags.isWebEnabled
            ? stringArgDeprecated('web-hostname')
            : '',
        port: featureFlags.isWebEnabled ? stringArgDeprecated('web-port') : '',
        webUseSseForDebugProxy: featureFlags.isWebEnabled &&
            stringArgDeprecated('web-server-debug-protocol') == 'sse',
        webUseSseForDebugBackend: featureFlags.isWebEnabled &&
            stringArgDeprecated('web-server-debug-backend-protocol') == 'sse',
        webUseSseForInjectedClient: featureFlags.isWebEnabled &&
            stringArgDeprecated('web-server-debug-injected-client-protocol') ==
                'sse',
        webEnableExposeUrl: featureFlags.isWebEnabled &&
            boolArgDeprecated('web-allow-expose-url'),
        webRunHeadless:
            featureFlags.isWebEnabled && boolArgDeprecated('web-run-headless'),
      ...
      return DebuggingOptions.enabled(
        ...
        webEnableExpressionEvaluation: featureFlags.isWebEnabled &&
            boolArgDeprecated('web-enable-expression-evaluation'),
        webLaunchUrl: featureFlags.isWebEnabled
            ? stringArgDeprecated('web-launch-url')
            : null,
...
class RunCommand extends RunCommandBase {
  ...
  DeprecationBehavior get deprecationBehavior =>
      boolArgDeprecated('ignore-deprecation')
  ...
  Future<CustomDimensions> get usageValues async {
    ...
    if (devices == null || devices!.isEmpty) {
      ...
      if (device is IOSDevice &&
          device.interfaceType == IOSDeviceConnectionInterface.network) {
  ...
  bool get awaitFirstFrameWhenTracing =>
      boolArgDeprecated('await-first-frame-when-tracing');
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    for (final Device device in devices!) {
      ...
      if (await device.isLocalEmulator &&
          await device.supportsHardwareRendering) {
    ...
    try {
      final int? result = await runner.run(
        ...
        enableDevTools:
            stayResident && boolArgDeprecated(FlutterCommand.kEnableDevTools),
        route: route,
      );
      handler?.stop();
      if (result != 0) {
