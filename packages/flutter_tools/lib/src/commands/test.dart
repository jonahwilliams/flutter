...
class TestCommand extends FlutterCommand with DeviceBasedDevelopmentArtifacts {
  ...
  Future<FlutterCommandResult> verifyThenRunCommand(String? commandPath) {
    _testFiles =
        argResults!.rest.map<String>(globals.fs.path.absolute).toList();
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    final int? shardIndex =
        int.tryParse(stringArgDeprecated('shard-index') ?? '');
    ...
    final int? totalShards =
        int.tryParse(stringArgDeprecated('total-shards') ?? '');
    ...
    final int result = await testRunner.runTests(
      ...
      enableObservatory: collector != null ||
          startPaused ||
          boolArgDeprecated('enable-vmservice'),
      ...
      integrationTestUserIdentifier:
          stringArgDeprecated(FlutterOptions.kDeviceUser),
      testTimeRecorder: testTimeRecorder,
    );
