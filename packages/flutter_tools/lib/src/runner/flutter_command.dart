...
abstract class FlutterCommand extends Command<void> {
  ...
  Uri? get devToolsServerAddress {
    if (argResults?.wasParsed(kDevToolsServerAddress) ?? false) {
      final Uri? uri =
          Uri.tryParse(stringArgDeprecated(kDevToolsServerAddress)!);
  ...
  Future<bool> get disablePortPublication async =>
      !boolArgDeprecated('publish-port');
  ...
  void useDartDefineConfigJsonFileOption() {
    argParser.addOption(FlutterOptions.kDartDefineFromFileOption,
        help:
  ...
  late final Duration? deviceDiscoveryTimeout = () {
    if ((argResults?.options.contains(FlutterOptions.kDeviceTimeout) ??
      ...
      final int? timeoutSeconds =
          int.tryParse(stringArgDeprecated(FlutterOptions.kDeviceTimeout)!);
  ...
  BuildMode getBuildMode() {
    ...
    final bool jitReleaseResult =
        !_excludeRelease && boolArgDeprecated('jit-release');
  ...
  Future<BuildInfo> getBuildInfo(
    ...
    if (argParser.options.containsKey(FlutterOptions.kAnalyzeSize) &&
        boolArgDeprecated(FlutterOptions.kAnalyzeSize)) {
      ...
      if (argParser.options.containsKey(FlutterOptions.kCodeSizeDirectory) &&
          stringArgDeprecated(FlutterOptions.kCodeSizeDirectory) != null) {
        directory = globals.fs
            .directory(stringArgDeprecated(FlutterOptions.kCodeSizeDirectory));
      }
    ...
    if (argParser.options.containsKey(FlutterOptions.kNullSafety)) {
      ...
      if (!wasNullSafetyFlagParsed &&
        ...
        if ((languageVersion.major > nullSafeVersion.major) ||
            (languageVersion.major == nullSafeVersion.major &&
    ...
    if (argParser.options.containsKey(FlutterOptions.kWebRendererFlag)) {
      final WebRendererMode? mappedMode = _webRendererModeMap[
          stringArgDeprecated(FlutterOptions.kWebRendererFlag)!];
      if (mappedMode != null) {
    ...
    if (argParser.options
      ...
      final String? configJsonPath =
          stringArg(FlutterOptions.kDartDefineFromFileOption);
      if (configJsonPath != null && globals.fs.isFileSync(configJsonPath)) {
        final String configJsonRaw =
            globals.fs.file(configJsonPath).readAsStringSync();
        try {
          ...
          throwToolExit(
              'Json config define file "--${FlutterOptions.kDartDefineFromFileOption}=$configJsonPath" format err, '
    ...
    return context.run<void>(
      ...
      body: () async {
        if (_usesFatalWarnings) {
          globals.logger.fatalWarnings =
              boolArgDeprecated(FlutterOptions.kFatalWarnings);
        }
  ...
  Future<List<Device>?> findAllTargetDevices({
    ...
    if (devices.isEmpty) {
      if (deviceManager.hasSpecifiedDeviceId) {
        globals.logger.printStatus(userMessages
      ...
      if (deviceManager.hasSpecifiedDeviceId) {
        globals.logger.printStatus(userMessages.flutterFoundSpecifiedDevices(
        ...
        if (globals.terminal.stdinHasTerminal) {
          ...
          final Device chosenDevice =
              await _chooseOneOfAvailableDevices(devices);
  ...
  void _displayDeviceOptions(List<Device> devices) {
    ...
    for (final Device device in devices) {
      globals.logger.printStatus(
          userMessages.flutterChooseDevice(count, device.name, device.id));
      count++;
    }
  }
