...
''');
    ...
    testUsingContext(
        'tool exits on non-sound-null-safe code when explicit flag not passed',
        () async {
      final DummyFlutterCommand flutterCommand =
    ...
    group('findAllTargetDevices', () {
      ...
      group('when specified device id', () {
        testUsingContext('returns device when device is found', () async {
          ...
          final List<Device>? devices =
              await flutterCommand.findAllTargetDevices();
        ...
        testUsingContext('show error when no device found', () async {
          ...
          expect(testLogger.statusText,
              contains(UserMessages().flutterNoMatchingDevice('device-id')));
        });
        ...
        testUsingContext('show error when multiple devices found', () async {
          ...
          expect(
              ...
              contains(
                  UserMessages().flutterFoundSpecifiedDevices(2, 'device-id')));
        });
      });
      ...
      group('when specified all', () {
        ...
        testUsingContext('show error when no device found', () async {
          ...
          expect(devices, null);
          expect(testLogger.statusText,
              contains(UserMessages().flutterNoDevicesFound));
        });
      });

      group('when device not specified', () {
        testUsingContext('returns one device when only one device connected',
            () async {
          testDeviceManager.addDevice(device1);
          final DummyFlutterCommand flutterCommand = DummyFlutterCommand();
          final List<Device>? devices =
        ...
        testUsingContext('show error when no device found', () async {
          ...
          expect(devices, null);
          expect(testLogger.statusText,
              contains(UserMessages().flutterNoSupportedDevices));
        });

        testUsingContext(
            'show error when multiple devices found and not connected to terminal',
            () async {
          testDeviceManager.addDevice(device1);
          testDeviceManager.addDevice(device2);
          final DummyFlutterCommand flutterCommand = DummyFlutterCommand();
          ...
          expect(devices, null);
          expect(testLogger.statusText,
              contains(UserMessages().flutterSpecifyDeviceWithAllOption));
        }, overrides: <Type, Generator>{
